...
const root = document.documentElement;
    ...
    const placeMobileNavigation = () => {
      const mainPane = document.querySelector('.main-pane');
      if (!mainPane) return;

      const firstPanel =
        mainPane.querySelector(':scope > .content-panel') ??
        mainPane.querySelector('.content-panel')?.closest('.main-pane > *') ??
        mainPane.firstElementChild;
      const titlePanel = mainPane.querySelector('.content-panel:first-of-type');
      const sectionNav = document.querySelector('.docs-mobile-section-nav');
      const tocWrapper = document.querySelector('.netscli-mobile-toc-wrapper');

      if (sectionNav && sectionNav.parentElement !== mainPane) {
        mainPane.insertBefore(sectionNav, firstPanel);
      }

      if (tocWrapper && titlePanel && tocWrapper.previousElementSibling !== titlePanel) {
        titlePanel.after(tocWrapper);
      }
    };

    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', placeMobileNavigation, { once: true });
    } else {
      placeMobileNavigation();
    }
    window.addEventListener('astro:page-load', placeMobileNavigation);
    window.addEventListener('resize', placeMobileNavigation);
    ...
    const enhanceAnchorLinks = () => {
      document.querySelectorAll('.sl-markdown-content .sl-anchor-link[href^="#"]').forEach((anchor) => {
        if (anchor.dataset.netscliAnchorBound === 'true') return;
        anchor.dataset.netscliAnchorBound = 'true';
        anchor.setAttribute('aria-label', 'Copy link to this section');
        anchor.addEventListener('click', async (event) => {
          event.preventDefault();
          const href = anchor.getAttribute('href') || '';
          const nextUrl = `${window.location.origin}${window.location.pathname}${href}`;
          try {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', enhanceAnchorLinks, { once: true });
    } else {
      enhanceAnchorLinks();
    }
    window.addEventListener('astro:page-load', enhanceAnchorLinks);
}
