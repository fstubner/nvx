...
func TestCaretAndTildeCeilings(t *testing.T) {
	for _, tc := range []struct {
		...
		// A partial caret holds the leftmost non-zero part among the parts
		// given, or the last part given when all of them are zero. The ceiling
		// used to be computed as if all three were given, so "^0" and "^0.0"
		// both meant "<0.0.1" and allowed nothing but 0.0.0.
		{"^0", []string{"0.0.0", "0.9.9"}, []string{"1.0.0"}},
		{"^0.0", []string{"0.0.0", "0.0.9"}, []string{"0.1.0"}},
		{"^0.0.x", []string{"0.0.0", "0.0.9"}, []string{"0.1.0"}},
		{"^0.2", []string{"0.2.0", "0.2.9"}, []string{"0.1.9", "0.3.0"}},
