...
func TestOneSandboxSessionCannotReadAnother(t *testing.T) {
	...
	scopeCaps, _, err := prepareAppContainerFilesystem(sid, "", victimHome, victimWork)
	if err != nil {
		t.Fatalf("victim session prep: %v", err)
	}
	scopeCaps, _, err = prepareAppContainerFilesystem(sid, "", toolHome, victimWork)
	if err != nil {
		t.Fatalf("tool session prep: %v", err)
	}

	// Session 3: an ordinary `npm install` in an unrelated project.
	scopeCaps, _, err = prepareAppContainerFilesystem(sid, "", attackerHome, attackerWork)
	if err != nil {
