...
func TestShouldSandbox(t *testing.T) {
	...
	// npm install is classInstall, which standard isolation contains by default.
	if !shouldSandbox("npm", []string{"install"}, policy, shimOptions{}) {
		t.Fatal("expected npm install to sandbox by default")
	}
	// A --no-sandbox smuggled through the wrapped command must NOT bypass.
	if !shouldSandbox("npm", []string{"install"}, policy, shimOptions{payloadNoSandbox: true}) {
