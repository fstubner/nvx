...
- **Thin wrapper**: A single static Go binary with no runtime dependencies. Each wrapped command runs through one extra short-lived process; resolved binary paths are cached (keyed by `PATH`) so the shim doesn't rescan `PATH` on every call. Measured dispatch overhead: **~3 ms on Linux, ~4 ms on macOS, and roughly 1–60 ms on Windows** — process creation is costlier there, and the figure moves far more than the other two, both between machines and between runs on one machine. Samples from [`scripts/bench.py`](scripts/bench.py) across several machines: 0.9, 2.7, 3.2, 3.4, 6.1, 8.6, 9, 11, 38 and 57 ms. Quote the range, not a number; two earlier versions of this line published a single figure and then a floor, and each was contradicted by the next machine to run the script. Imperceptible either way next to the commands you actually wait on, like `npm install`.
